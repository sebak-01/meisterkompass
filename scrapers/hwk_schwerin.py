"""Scraper for HWK Schwerin's ODAV Meister course catalogue."""
...
BASE_URL = "https://www.hwk-schwerin.de"
LIST_URL = (
    f"{BASE_URL}/kurse/liste-19,0,courselist.html?search-filter-template=0&search-type=6"
)
EXAM_FEES_PAGE_URL = (
    f"{BASE_URL}/artikel/aufgabenerfuellung-als-oeffentlich-rechtliche-koerperschaft-in-selbstverwaltung-19,95,763.html"
)
FEES_PDF_URL = f"{BASE_URL}/downloads/gebuehrenverzeichnis-19,8.pdf"
GENERIC_EXAM_FEES = {1: 400.0, 2: 400.0, 3: 200.0, 4: 200.0}

SCHWERIN_TRADE_ALIASES = {
    "metallbau": "Metallbauer",
    "kfz- techniker": "Kfz.-Techniker",
    "kfz techniker": "Kfz.-Techniker",
    "inst./hzg": "Installateur- und Heizungsbauer",
    "inst./ hzg": "Installateur- und Heizungsbauer",
...
LOCATIONS = {
    "schwerin": ("Werkstraße 6", "19061", "Schwerin"),
    "güstrow": ("", "18273", "Güstrow"),
    "guestrow": ("", "18273", "Güstrow"),
}


def parse_schwerin_title(title: str) -> tuple[list[int], str | None]:
    ...
    contexts = (
        title,
        title.replace("Meisterausbildung", "Meister Meisterausbildung"),
        f"Meister {title}",
    )
    trade = None
    ...
    if not trade:
        lower = title.lower()
        for source, canonical in SCHWERIN_TRADE_ALIASES.items():
            if source in lower:
                trade = canonical
                break
...
def _availability(text: str) -> str:
    ...
    if any(
        ...
        return "available"
    return parse_availability(text)


class HwkSchwerinScraper(BavariaOdavScraper):
    chamber_slug = "hwk-schwerin"
    chamber_name = "Handwerkskammer Schwerin"
    ...
    catalogue = BavariaCatalogue(
        ...
        default_city="Schwerin",
        default_street="Werkstraße 6",
        default_zip="19061",
        page_size=100,
    ...
    def _parse_card(self, link: Tag, detail_url: str | None = None) -> dict | None:
        ...
        parts, trade_name = parse_schwerin_title(raw_title)
        if not parts:
    ...
    def _enrich(self, card: dict) -> RawCourseOffer | list[RawCourseOffer] | None:
        ...
        if soup is not None:
            ...
            parts, trade_name = parse_schwerin_title(detail_title)
            if parts:
    ...
    def parse_meister_exam_fees(text: str) -> dict[int, float]:
        fees: dict[int, float] = {}
        patterns = (
            (1, r"8\.5\.1\.\s+Teil\s+I\s+([\d.]+),(\d{2})"),
            (2, r"8\.5\.2\.\s+Teil\s+II\s+([\d.]+),(\d{2})"),
            (3, r"8\.5\.3\.\s+Teil\s+III\s+([\d.]+),(\d{2})"),
            (4, r"8\.5\.4\.\s+Teil\s+IV\s+([\d.]+),(\d{2})"),
        )
    ...
    def _resolve_exam_fees_pdf_url(self) -> str:
        ...
        for link in soup.select("a[href*='gebuehrenverzeichnis']"):
            href = link.get("href", "")
            if href.lower().endswith(".pdf") and "aenderung" not in href.lower():
    ...
    def _fetch_exam_fees_from_pdf(self) -> dict[int, float]:
        try:
            ...
            logger.warning("HWK Schwerin: pypdf not installed — using fallback exam fees.")
            return {}

        pdf_url = self._resolve_exam_fees_pdf_url()
        response = self.get(pdf_url)
        if response is None:
            logger.warning("HWK Schwerin: could not fetch exam-fee PDF.")
            return {}
        ...
        if not fees:
            logger.warning("HWK Schwerin: could not parse Meister exam fees from PDF.")
        return fees
